...
- **Tags to Search (Pre)**: This add the tags you define (this should be separated by commas e.g: 1girl,solo,short_hair) to the search query. This is useful if you want to get tags from a specific category, like "1girl" or "solo".
      Additionally, this field now supports a wildcard feature. You can use `__keyword__` (e.g., `__artist__` or `__character_style__`) in your tags. If a corresponding `keyword.txt` file (e.g., `artist.txt`) exists in the `user/wildcards/` directory, the script will randomly select one line (one tag) from that file to replace the `__keyword__` placeholder. If the file doesn't exist or is empty, the wildcard will be removed. Each `__keyword__` instance is processed independently, allowing for multiple, potentially different, random selections from the same or different wildcard files.
